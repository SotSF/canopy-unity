...
#if UNITY_EDITOR
        // With Fast Enter Play Mode (Domain Reload disabled), static fields
        // retain their values across play-mode sessions, so a stale MidiDriver
        // instance from a previous session can linger. SubsystemRegistration
        // runs on every play entry; dispose any stale driver and rebuild a
        // fresh one so the Editor path (which otherwise only re-creates the
        // driver on domain reload via AssemblyReloadEvents) stays correct. The
        // Player path builds its own driver in Initialize(), so this is
        // Editor-only to avoid creating a second driver there.
