...
namespace WebSocketServer {
    ...
    class WebSocketProtocol {
        ...
        public static byte[] DecodeBytes(WebSocketDataFrame dataframe)
        {
            if (dataframe.length > 0 && dataframe.mask)
            {
                ...
                {
                    var dataoffset = payloadOffset + i;
                    var encodedbyte = dataframe.data[dataoffset];
                    var maskbyte = masks[i % 4];
                    decoded[i] = (byte)(encodedbyte ^ maskbyte);
                }
