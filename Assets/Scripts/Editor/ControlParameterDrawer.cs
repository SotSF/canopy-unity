...
public class PatternParameterDrawer : PropertyDrawer
{
    ...
    public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
    {
        ...
        var nameRect = new Rect(position.x, position.y, 3* width / 4, rowHeight);
        var typeRect = new Rect(position.x + 3* width / 4, position.y, width / 4, rowHeight);
        ...
        if (IsNumeric(param))
        {
            var rangeRect = new Rect(position.x, position.y+ rowSpacing, width, rowHeight);
            var rangeProp = property.FindPropertyRelative("useRange");
            EditorGUI.PropertyField(rangeRect, rangeProp);
            if (param.useRange)
            {
                var minRect = new Rect(position.x, position.y + 2* rowSpacing, width / 2, rowHeight);
                var maxRect = new Rect(position.x + width / 2, position.y + 2* rowSpacing, width / 2, rowHeight);
        ...
        var defaultRect = new Rect(position.x, position.y + ((rows-1) * rowSpacing), position.width, rowHeight);
